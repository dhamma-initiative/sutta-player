...
fs.readdir(directoryPath, (err, files) => {
  ...
  txtFiles.forEach((txtFile) => {
    ...
    let pos = data.indexOf('Majjhima Nikāya')
    const substring = capitalizeWordsWithHyphens(data.substring(0, pos).trim())

    const modifiedString = substring.replace(/\(([^)]+)\)/g, (match, content) => {
      return '(' + content.charAt(0).toUpperCase() + content.slice(1) + ')';
    });

    // Output filename and substring as comma-delimited fields
    console.log(`${txtFile}, ${modifiedString}`);
  });
});
