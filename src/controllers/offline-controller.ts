...
export class OfflineController {
    ...
    private async _onDownloadAlbum(baseRefs?: string[]): Promise<void> {
        if (!baseRefs) {
    ...
    private async _onRemoveAlbum(baseRefs?: string[]): Promise<void> {
        if (!baseRefs) {
            baseRefs = await this._mainCtrl._albumStore.queryTrackReferences(this._model.catSel.albumIndex)
            const basePath = this._model.catSel.dictionary['albumRef']
            for (let i = 0; i < baseRefs.length; i++) 
    ...
    private async _handleOperationExit(finalStatusMsg: string, dwnDelDisable: boolean[], dwnDelChecked: boolean[]): Promise<void> {
